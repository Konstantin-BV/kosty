...
namespace UnitTest1
{
	...
	public:
		...
		TEST_METHOD(map)
		{
			...
			tree.Insert(NULL, tree.root, pm);
			tree.Insert(NULL, tree.root, k);
			Assert::AreEqual(tree.associa.VALUE(tree.find(pm)->key), q);
			Assert::AreEqual(tree.associa.VALUE(tree.find(k)->key), w);
			Assert::AreEqual(tree.associa.VALUE(tree.find(l)->key), i);
